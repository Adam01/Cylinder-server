...
class ResponseBase:
    def __init__(self, id, method, status):
        self.id = id
        self.method = method
        self.status = status
...
class TaskError(ResponseBase, JSONCallableEscape):
    def __init__(self, id, method, err):
...
class TaskProgressing(ResponseBase):
    def __init__(self, id, method, progress, data):
...
class TaskCompleted(ResponseBase):
    def __init__(self, id, method, result, data):
...
class CopyTaskHandler:
    def __init__(self, fsprocs, id, method, source, target, total):
    ...
    def on_enter_dir(self, original_dir, target_dir, target_path_dir, target_name):
        data = dict()
        data["source"] = self.source
        data["target"] = self.target
        data["msg"] = "Copying directory: '%s' to '%s'" % ( original_dir.get_path(), target_dir.get_path() )
        task = TaskProgressing(self.id, self.method, self.count / self.total, data)
        self.fsprocs.notify("Tasks", task.__dict__)

    def on_copied_file(self, original_dir, target_dir, target_path_dir, original, copied):
        self.count += 1
        data = dict()
        data["source"] = self.source
        data["target"] = self.target
        data["msg"] = "Copied file: '%s' to '%s'" % ( original.get_path(), copied.get_path() )
        task = TaskProgressing(self.id, self.method, self.count / self.total, data)
        self.fsprocs.notify("Tasks", task.__dict__)

    def perform_copy(self, entity, target_dir, target_name):
        on_enter_dir = functools.partial(reactor.callFromThread, self.on_enter_dir)
        on_copied_file = functools.partial(reactor.callFromThread, self.on_copied_file)
        ...
        reactor.callLater(0, self.fsprocs.notify, "Tasks",
                          TaskCompleted(self.id, self.method, True,
                                        {"source": self.source, "target": self.target, "count": self.count}
                          ).__dict__
        )


    def initialise(self, entity, target_dir, target_name):
        reactor.callInThread(self.perform_copy, entity=entity, target_dir=target_dir, target_name=target_name)
        return TaskInitiated(self.id, self.method, {"source": self.source, "target": self.target, "total": self.total})
...
class FileSystemProcedures(JSONCallable, EventSubject):
    ...
    def complete_task(self, result, data):
        return TaskCompleted(self.current_id, self.current_method, result, data)
    ...
    def jsonrpc_copy_entity(self, source, target):
        ...
        if isinstance(entity, FileSystemDirectory):
            ...
            handler = CopyTaskHandler(self, self.current_id, self.current_method, source, target, total)
