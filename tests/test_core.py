...
'''
C.02 The system is able to authenticate supplied credentials on both Windows
and Linux operating systems.
The test will check the result of an authentication function, ensuring it
returns null
...
# "C.02 TestUserAuthentication: Cannot test user authentication on "
# "Travis CI")
...
class C02TestUserAuthentication(unittest.TestCase):
    ...
    '''
    Test authentication class exists and is capable of constructing when
    ...
    def test_user_authentication(self):
        ...
        self.assertRaises(LoginError, Authentication, self.incorrect_username,
                          self.incorrect_password)

    @unittest.skipIf(
        sys.platform.startswith("win"),
        "C.02 test_user_authentication_pam: PAM not supported on windows"
    )
...
The test is successful once a process has been created and confirmed by
outputting its current operating user.
'''


@unittest.skipIf(
    os.environ.get("TEST_USER") is None or
    os.environ.get("TEST_PASSWORD") is None or
    sys.platform.startswith("win"),
    "TestSpawnUserProcess: No correct test user supplied || "
    "Needs windows service user"
)
